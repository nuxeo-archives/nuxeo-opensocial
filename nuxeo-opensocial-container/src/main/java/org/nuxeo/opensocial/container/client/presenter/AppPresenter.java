...
public class AppPresenter extends WidgetPresenter<AppPresenter.Display> {
    ...
    @Inject
    public AppPresenter(Display display, EventBus eventBus, ContainerPresenter containerPresenter,
            Provider<ContainerBuilderPresenter> containerBuilderPresenter, AppModel model) {
    ...
    protected void onBind() {
        eventBus.fireEvent(new SendMessageEvent(infos.isLoading(), Severity.SUCCESS, true));
    ...
    private void registerExternalPortletUpdateEvent() {
        registerHandler(eventBus.addHandler(UpdateWebContentEvent.TYPE, new UpdateWebContentEventHandler() {
            public void onUpdateWebContent(UpdateWebContentEvent event) {
                model.updateWebContent(event.getWebContentId(), event.getFiles());
            }
        }));
    }
    ...
    private void registerLayoutLoadEvent() {
        registerHandler(eventBus.addHandler(LayoutLoadedEvent.TYPE, new LayoutLoadedEventHandler() {
    ...
    private void setOpenSocialWebContentTitle(String frameId, String title) {
        ...
        try {
            ...
            ((PortletWidget) containerPresenter.getDisplay().getWebContent(webContentId)).setTitle(title);
    ...
    private String getGadgetUrlByGadgetId(String frameId) {
        try {
            ...
            eventBus.fireEvent(new SendMessageEvent(errors.cannotFindWebContent(), Severity.ERROR));
            return null;
        }
    }
    ...
    private void setOpenSocialUserPref(String frameId, String name, String value) {
        try {
            ...
            if (userPref != null) {
                ...
                eventBus.fireEvent(new SendMessageEvent(errors.preferenceDoesNotExist(name), Severity.ERROR));
            }
        } catch (ClassCastException e) {
            eventBus.fireEvent(new SendMessageEvent(errors.cannotFindWebContent(), Severity.ERROR));
        }
    ...
    private void addGadget(String type, JSParams<?> params) {
        ...
        if (webContent != null && webContent.initPrefs(prefs) == true) {
            ...
            eventBus.fireEvent(new SendMessageEvent(errors.cannotAddExternalWebContent(type), Severity.ERROR));
        }
    }

    @SuppressWarnings("unused")
    private void openContainerBuilder() {
        eventBus.fireEvent(new SendMessageEvent(infos.isLoading(), Severity.INFO, true));
        if (containerBuilderBinded == false) {
            GWT.runAsync(new RunAsyncCallback() {
                ...
                public void onFailure(Throwable reason) {
                    eventBus.fireEvent(new SendMessageEvent(errors.cannotLoadContainerBuilder(), Severity.ERROR));
                }
            });
        } else {
            containerBuilderPresenter.get().revealDisplay();
