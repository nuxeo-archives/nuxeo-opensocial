...
/**
 * This class is just a wrapper to hold the initialization of the shindig AuthenticationServletFilter because it cannot
 * run without Guice and we have delayed the Guice initialization.
 *
...
public class AuthenticationFilterDelayer implements Filter {
    ...
    /*
     * This is the true authentication filter but we have to do some work to delay its Guice-based initialization.
     */
    ...
    /*
     * In some packagings (jetty) this object is initialized AFTER the framework is ready, so we need to know if the
     * framework is ready before us.
     */
    protected static boolean hasBeenActivated = false;

    /*
     * We do not create this object, the web container does so we are forced to keep track of all the objects that are
     * so created. This list should be of size 1, but we handle any number the same way.
     */
    ...
    /*
     * In normal operation, this is just a pass through to the wrapped and "true" AuthenticationFilter.
    ...
    /*
     * Do the work that should have happened init time. This also "turns on" the filter so future calls pass directly
     * through this class to the wrapped filter.
     */
    ...
    /*
     * Note: This is static! This echos the framework event to all the instances of this class.
