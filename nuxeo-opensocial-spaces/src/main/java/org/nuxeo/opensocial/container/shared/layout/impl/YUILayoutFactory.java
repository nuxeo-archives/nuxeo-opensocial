...
public class YUILayoutFactory implements Serializable {
    ...
    public static YUILayout createLayout(YUIAbstractBodySize size, boolean header, boolean footer,
            YUISideBarStyle sideBar) {
        return new YUILayoutImpl(size, header, footer, sideBar);
    }

    public static YUILayout createDummies() {
        YUILayout layout = YUILayoutFactory.createLayout(new YUIFixedBodySize(YUISize.YUI_BS_FULL_PAGE), true, true,
                YUISideBarStyle.YUI_SB_NO_COLUMN);

        YUIAbstractComponent zone1 = new YUIComponentZoneImpl(YUITemplate.YUI_ZT_100);
        layout.getContent().addComponent(zone1);

        ((YUIComponentZoneImpl) zone1).addComponent(new YUIUnitImpl());

        YUIAbstractComponent zone2 = new YUIComponentZoneImpl(YUITemplate.YUI_ZT_25_75);
